...
\includegraphics{Rysunki/figures/webshot_3-1.png}
...
\CommentTok{\# install\_github("pablobarbera/scholarnetwork")}

\FunctionTok{library}\NormalTok{(igraph)}
\FunctionTok{library}\NormalTok{(scholarnetwork)}
\FunctionTok{library}\NormalTok{(ggplot2)}

\NormalTok{d }\OtherTok{\textless{}{-}} \FunctionTok{extractNetwork}\NormalTok{(}\AttributeTok{id=}\StringTok{"RNDE9{-}wAAAAJ"}\NormalTok{, }\AttributeTok{n=}\DecValTok{50}\NormalTok{)}
...
##   ..$ degree: num [1:53] 0.667 0.667 4.65 2.667 2.833 ...
##   ..$ group : num [1:53] 7 12 1 8 2 2 5 6 2 7 ...
...
\NormalTok{l }\OtherTok{\textless{}{-}} \FunctionTok{layout.fruchterman.reingold}\NormalTok{(network, }\AttributeTok{niter=}\DecValTok{1500}\NormalTok{) }\CommentTok{\# layout}
...
\NormalTok{pq }\OtherTok{\textless{}{-}}\NormalTok{ p }\SpecialCharTok{+} \FunctionTok{geom\_text}\NormalTok{(}\AttributeTok{color=}\StringTok{"black"}\NormalTok{, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{label=}\NormalTok{label, }\AttributeTok{size=}\NormalTok{degree),}
  ...
  \FunctionTok{geom\_segment}\NormalTok{(}
    ...
    \AttributeTok{data=}\NormalTok{edges, }\AttributeTok{size=}\FloatTok{0.25}\NormalTok{, }\AttributeTok{color=}\StringTok{"grey20"}\NormalTok{, }\AttributeTok{alpha=}\DecValTok{1}\SpecialCharTok{/}\DecValTok{5}\NormalTok{) }\SpecialCharTok{+}
  ...
  \FunctionTok{theme}\NormalTok{(}
    \AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{size =} \DecValTok{20}\NormalTok{, }\AttributeTok{color =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{face =} \StringTok{"bold"}\NormalTok{, }\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{),}
...
\NormalTok{  ) }\SpecialCharTok{+}
  \DocumentationTok{\#\# changing size of points in legend}
...
\hypertarget{analiza-poruxf3wnujaca-iloux15bci-cytowaux144-dla-publikacji-napisanych-przez-naukowcuxf3w-w-kolejnych-latach-ich-pracy}{%
\subsubsection{Analiza porównujaca ilości cytowań dla publikacji
napisanych przez naukowców w kolejnych latach ich
pracy}\label{analiza-poruxf3wnujaca-iloux15bci-cytowaux144-dla-publikacji-napisanych-przez-naukowcuxf3w-w-kolejnych-latach-ich-pracy}}
...
\hypertarget{analiza-dynamiki-rozwoju-publikacyjnego-wybranych-naukowcuxf3w-na-podstawie-iloux15bci-skumulowanych-cytowaux144-w-zaleznoux15bci-od-dux142ugoux15bci-staux17cu-pracy}{%
\subsubsection{Analiza dynamiki rozwoju publikacyjnego wybranych
naukowców na podstawie ilości skumulowanych cytowań w zalezności od
długości stażu
pracy}\label{analiza-dynamiki-rozwoju-publikacyjnego-wybranych-naukowcuxf3w-na-podstawie-iloux15bci-skumulowanych-cytowaux144-w-zaleznoux15bci-od-dux142ugoux15bci-staux17cu-pracy}}
...
\NormalTok{ids }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"RNDE9{-}wAAAAJ\&hl"}\NormalTok{, }\StringTok{"jkj3pCQAAAAJ\&hl"}\NormalTok{,}\StringTok{"L6MYKCQAAAAJ\&hl"}\NormalTok{ , }\StringTok{"XNRUNHsAAAAJ\&hl"}\NormalTok{)}
...
\NormalTok{p }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(df\_3, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ career\_year, }\AttributeTok{y =}\NormalTok{ cumulative\_cites)) }\SpecialCharTok{+}
  \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{colour =}\NormalTok{ name)) }\SpecialCharTok{+}
  \FunctionTok{scale\_fill\_brewer}\NormalTok{(}\AttributeTok{palette =} \StringTok{"BrBG"}\NormalTok{)}\SpecialCharTok{+}
  \FunctionTok{geom\_text}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{label=}\NormalTok{cumulative\_cites), }\AttributeTok{size=}\DecValTok{3}\NormalTok{)}\SpecialCharTok{+}
...
\hypertarget{google-scholar---metody-web-scraping}{%
\subsubsection{Google Scholar - metody web
scraping}\label{google-scholar---metody-web-scraping}}
...
RNDE9-wAAAAJ\&hl) w GS. Moduł BeautifulSoup wykorzystuje się do
parsowania i przeszukiwania dokumentów HTML.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# import modułów}
\ImportTok{import}\NormalTok{ pandas }\ImportTok{as}\NormalTok{ pd }\CommentTok{\#biblioteka do pracy z danymi tabelarycznymi}
\ImportTok{import}\NormalTok{ requests }\CommentTok{\#biblioteka do wykonywania zapytań HTTP}
\ImportTok{from}\NormalTok{ bs4 }\ImportTok{import}\NormalTok{ BeautifulSoup }\ImportTok{as}\NormalTok{ bs }\CommentTok{\#przetwarzania dokumentów HTML}
\ImportTok{from}\NormalTok{ tabulate }\ImportTok{import}\NormalTok{ tabulate }\CommentTok{\#wyświetlania danych w postaci tabel}
\ImportTok{from}\NormalTok{ rich.console }\ImportTok{import}\NormalTok{ Console}

\NormalTok{pd.set\_option(}\StringTok{\textquotesingle{}display.max\_columns\textquotesingle{}}\NormalTok{, }\VariableTok{None}\NormalTok{) }\CommentTok{\#kolumny w ramkach danych pandas}
\NormalTok{pd.set\_option(}\StringTok{\textquotesingle{}display.max\_colwidth\textquotesingle{}}\NormalTok{, }\DecValTok{65}\NormalTok{) }\CommentTok{\#maksymalnej szerokości kolumny}
\NormalTok{big\_df }\OperatorTok{=}\NormalTok{ pd.DataFrame()}\CommentTok{\# inicjalizacja pustej ramki danych}
\CommentTok{\# definicja nagłówków i sesji HTTP}
...
\ControlFlowTok{for}\NormalTok{ x }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{500}\NormalTok{, }\DecValTok{100}\NormalTok{): }\CommentTok{\# iteracja x po zakresie od 0 do 500 z krokiem 100}
...
\NormalTok{    r }\OperatorTok{=}\NormalTok{ s.post(url, data}\OperatorTok{=}\NormalTok{payload)}
    \CommentTok{\# zastosowanie parsera html}
\NormalTok{    soup }\OperatorTok{=}\NormalTok{ bs(r.json()[}\StringTok{\textquotesingle{}B\textquotesingle{}}\NormalTok{], }\StringTok{\textquotesingle{}html.parser\textquotesingle{}}\NormalTok{) }\CommentTok{\#analiza strony przy użyciu BeautifulSoup}
\NormalTok{    works }\OperatorTok{=}\NormalTok{ [(x.get\_text(), }\StringTok{\textquotesingle{}https://scholar.google.com\textquotesingle{}} \OperatorTok{+}\NormalTok{ x.get(}\StringTok{\textquotesingle{}href\textquotesingle{}}\NormalTok{))}
             ...
             \KeywordTok{and} \BuiltInTok{len}\NormalTok{(x.get\_text()) }\OperatorTok{\textgreater{}} \DecValTok{7}\NormalTok{] }\CommentTok{\#pobranie informacji dla elementów HTML}
\CommentTok{\# utworzenie ramki danych z pobranych informacji}
...
\CommentTok{\# wyświetlenie wyników w postaci tabeli}
...
## 8  Wplyw nawozenia siarka elementarna na zawartosc mikroelemento...   
## 9  Zawartosc mikroelementow w ziarnie i slomie wybranych odmian ...   
## 
##                                                                Link  
...
\NormalTok{filePath }\OtherTok{\textless{}{-}} \StringTok{"siarka.txt"}
...
\includegraphics{Rysunki/figures/wordcloud-1.pdf}
...
Poniżej przedstawiono przeszukiwanie bazy danych Semantic Scholar przy
wykorzystaniu własnego klucza API otrzymanego po zaakceptowaniu wniosku
przez dostawcę usług.

\hypertarget{analiza-publikacji-dla-2-autoruxf3w-za-pomocux105-biblioteki-pys2-python-library-for-the-semantic-scholar-api.}{%
\subsubsection{\texorpdfstring{Analiza publikacji dla 2 autorów za
pomocą biblioteki \href{https://github.com/mirandrom/PyS2}{PyS2: Python
Library for the Semantic Scholar
API}.}{Analiza publikacji dla 2 autorów za pomocą biblioteki PyS2: Python Library for the Semantic Scholar API.}}\label{analiza-publikacji-dla-2-autoruxf3w-za-pomocux105-biblioteki-pys2-python-library-for-the-semantic-scholar-api.}}

\begin{Shaded}
\begin{Highlighting}[]

\CommentTok{\# import s2}
\CommentTok{\# from requests import Session}
\CommentTok{\# session = Session()}
\CommentTok{\# session.headers = \{\textquotesingle{}x{-}api{-}key\textquotesingle{}: "prvXStDFku731CaJlUdWXa0aJtFKCOCU1FbdGCMs"\}}
\CommentTok{\# \# Pobranie pierwszego autora}
\CommentTok{\# author1 = s2.api.get\_author(authorId="87266064")}
\CommentTok{\# \#print("Author 1:")}
\CommentTok{\# \#print("Name:", author1.name)}
\CommentTok{\# paperIds1 = [p.paperId for p in author1.papers]}
\CommentTok{\# print("\textbackslash{}nPapers for Author 1:")}
\CommentTok{\# papers\_author1 = []}
\CommentTok{\# for pid in paperIds1:}
\CommentTok{\#     paper = s2.api.get\_paper(}
\CommentTok{\#         paperId=pid,}
\CommentTok{\#         retries=2,}
\CommentTok{\#         wait=300,}
\CommentTok{\#         params=dict(include\_unknown\_references=True))}
\CommentTok{\#     papers\_author1.append(paper)}
\CommentTok{\# for i, paper in enumerate(papers\_author1[:5]):  \# Wyświetlenie tylko 5 pierwszych rekordów dla autora 1}
\CommentTok{\#     print(f"  \{i+1\}. Title:", paper.title)}
\CommentTok{\#     print("     Authors:", ", ".join(author.name for author in paper.authors))}
\CommentTok{\#     print()}
\CommentTok{\# \# Pobranie drugiego autora}
\CommentTok{\# author2 = s2.api.get\_author(authorId="12961311")}
\CommentTok{\# \# print("\textbackslash{}nAuthor 2:")}
\CommentTok{\# \# print("Name:", author2.name)}
\CommentTok{\# paperIds2 = [p.paperId for p in author2.papers]}
\CommentTok{\# print("\textbackslash{}nPapers for Author 2:")}
\CommentTok{\# papers\_author2 = []}
\CommentTok{\# for pid in paperIds2:}
\CommentTok{\#     paper = s2.api.get\_paper(}
\CommentTok{\#         paperId=pid,}
\CommentTok{\#         retries=2,}
\CommentTok{\#         wait=150,}
\CommentTok{\#         params=dict(include\_unknown\_references=True))}
\CommentTok{\#     papers\_author2.append(paper)}
\CommentTok{\# for i, paper in enumerate(papers\_author2[:5]):  \# Wyświetlenie tylko 5 pierwszych rekordów dla autora 2}
\CommentTok{\#     print(f"  \{i+1\}. Title:", paper.title)}
\CommentTok{\#     print("     Authors:", ", ".join(author.name for author in paper.authors))}
\CommentTok{\#     print()}
\CommentTok{\# print("Liczba wszystkich publikacji dla Author 1:", len(papers\_author1))}
\CommentTok{\# print("Liczba wszystkich publikacji dla Author 2:", len(papers\_author2))}
\end{Highlighting}
\end{Shaded}

\hypertarget{przykux142ad-przeszukiwanie-bazy-danych-za-pomocux105-sux142uxf3w-kluczowych-z-pomoca-biblioteki-semanticscholar.}{%
\subsubsection{\texorpdfstring{Przykład przeszukiwanie bazy danych za
pomocą słów kluczowych z pomoca biblioteki
\href{https://pypi.org/project/semanticscholar/}{semanticscholar}.}{Przykład przeszukiwanie bazy danych za pomocą słów kluczowych z pomoca biblioteki semanticscholar.}}\label{przykux142ad-przeszukiwanie-bazy-danych-za-pomocux105-sux142uxf3w-kluczowych-z-pomoca-biblioteki-semanticscholar.}}

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{from}\NormalTok{ semanticscholar }\ImportTok{import}\NormalTok{ SemanticScholar}
\CommentTok{\# Ustaw klucz API}
\NormalTok{api\_key }\OperatorTok{=} \StringTok{"prvXStDFku731CaJlUdWXa0aJtFKCOCU1FbdGCMs"}
\CommentTok{\# Utwórz obiekt SemanticScholar z użyciem klucza API}
\NormalTok{sch }\OperatorTok{=}\NormalTok{ SemanticScholar(api\_key}\OperatorTok{=}\NormalTok{api\_key)}
\CommentTok{\# Zdefiniuj zapytanie}
\NormalTok{query }\OperatorTok{=} \StringTok{\textquotesingle{}elemental sulphur fertilizer\textquotesingle{}}
\CommentTok{\# Przeszukaj bazę danych Semantic Scholar}
\NormalTok{results }\OperatorTok{=}\NormalTok{ sch.search\_paper(query)}
\CommentTok{\# Wyświetl pierwsze 5 wyników}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{results}\SpecialCharTok{.}\NormalTok{total}\SpecialCharTok{\}}\SpecialStringTok{ results.\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(}\StringTok{"First 5 occurrences:"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{if}\NormalTok{ results.total }\OperatorTok{\textgreater{}} \DecValTok{0}\NormalTok{:}
    \ControlFlowTok{for}\NormalTok{ i, paper }\KeywordTok{in} \BuiltInTok{enumerate}\NormalTok{(results[:}\DecValTok{5}\NormalTok{], }\DecValTok{1}\NormalTok{):}
\NormalTok{        authors }\OperatorTok{=}\NormalTok{ [author.name }\ControlFlowTok{for}\NormalTok{ author }\KeywordTok{in}\NormalTok{ paper.authors]}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\SpecialCharTok{\{}\NormalTok{i}\SpecialCharTok{\}}\SpecialStringTok{. Title: }\SpecialCharTok{\{}\NormalTok{paper}\SpecialCharTok{.}\NormalTok{title}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"    Authors: }\SpecialCharTok{\{}\StringTok{\textquotesingle{}, \textquotesingle{}}\SpecialCharTok{.}\NormalTok{join(authors)}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
        \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"    Year: }\SpecialCharTok{\{}\NormalTok{paper}\SpecialCharTok{.}\NormalTok{year}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
##     Year: 1980
## 3. Title: INFLUENCE OF NITROGEN FERTILIZER AND ELMENTAL SULPHUR LEVELS ON PRODUCTIVITY AND TECHNOLOGICAL CHARACTERISTICS OF SUGAR BEET UNDER MIDDLE EGYPT CONDITIONS
##     Authors: M. A. Awad-Allah, E. A. A. E. Lattief, M. S. Ahmed
##     Year: 2022
## 4. Title: Effect of Bio sulphur granules (BSG) as fertilizer ingredient on different fractions of sulphur in calcareous soil cultivated with blackgram (Var.VBN-8)
##     Authors: G. Akila, S. Thiyageshwari, D. Selvi, R. Anandham, M. Djanaguiraman
##     Year: 2022
## 5. Title: Soil nutrients and maize responses to elemental sulphur as a fertilizer and soil amendment for bintang series soil
...
\item
  Wykorzystania kilku narzędzi data science (np. programy Python, R,
